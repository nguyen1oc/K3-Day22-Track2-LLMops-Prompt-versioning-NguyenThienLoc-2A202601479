...
PROMPT_V1_NAME = "nguyenloc-rag-prompt-v1"
PROMPT_V2_NAME = "nguyenloc-rag-prompt-v2"


# ── 2. Định nghĩa 2 Prompt Templates ──────────────────────────────────────
SYSTEM_V1 = (
    "Bạn là trợ lý AI thân thiện. Chỉ dùng context sau để trả lời. "
    "Trả lời ngắn gọn, trực tiếp (2-4 câu). Nếu không tìm thấy thông tin trong context, "
...
SYSTEM_V2 = (
    "Bạn là chuyên gia AI phân tích thông tin. Dựa vào context được cung cấp, "
    "hãy tổng hợp câu trả lời rõ ràng, có cấu trúc chặt chẽ và chuyên nghiệp (3-5 câu). "
    "Trích dẫn các luận điểm chính từ context. Nếu thông tin không có trong context, "
    "hãy nêu rõ rằng context không cung cấp thông tin này.\n\nContext:\n{context}"
)
...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(PROMPT_V2_NAME, object=PROMPT_V2, description="V2 – có cấu trúc, chuyên nghiệp")
...
def main():
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        if version_tag == "v1":
            ...
            v2_count += 1
        print(f"[{i+1:02d}] [{request_id}] [prompt-{version_tag}] Q: {question[:55]}...")
