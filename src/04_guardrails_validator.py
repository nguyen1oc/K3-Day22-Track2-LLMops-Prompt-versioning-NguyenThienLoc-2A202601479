...
class JSONFormatter(Validator):
    ...
    def _repair(text: str) -> str:
        ...
        # Thay single quotes bao quanh key và value thành double quotes
        text = text.replace("'", '"')
        # Xóa trailing commas trước } hoặc ]
    ...
    def validate(self, value: str, metadata: dict):
        ...
        Nếu hợp lệ → PassResult()
        Nếu sửa được → FailResult(fix_value=json_đã_sửa)
        Nếu không thể sửa → FailResult(fix_value=fallback_json)
        """
        try:
            json.loads(value)
            return PassResult()
        ...
        try:
            ...
            return FailResult(
                error_message="JSON cần được định dạng lại",
                fix_value=formatted,
            )
        except json.JSONDecodeError:
            pass
        ...
        fallback = json.dumps({
            "error": "Không thể phân tích cú pháp JSON",
            "raw": value[:100],
        }, ensure_ascii=False, indent=2)
        return FailResult(
            error_message="JSON hoàn toàn không hợp lệ, dùng fallback",
            fix_value=fallback,
        )
